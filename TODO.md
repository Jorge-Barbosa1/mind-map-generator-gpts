...
- [ ] **Revogar a API key do OpenRouter** atualmente em `backend/.env` e gerar uma nova. *(ação do utilizador)*
...
- [ ] Frontend: ativar `eslint` + `prettier` com config consistente; adicionar script `npm run lint`.
- [ ] `.editorconfig` na raiz para uniformizar indentação/encoding.
- [ ] `Makefile` (ou `justfile`) com alvos `dev`, `test`, `lint`, `format`, `build`.

### CI
- [ ] GitHub Actions com jobs: lint, type-check, testes (backend + frontend), build frontend. A correr em PRs para `main`.
