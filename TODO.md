...
- [x] Corrigir typo `reponse` → `response` em `backend/app/services/llm_client.py`.
- [x] Timeout aplicado no cliente OpenRouter (`settings.llm_timeout_seconds`). Retry com backoff (`tenacity`) ainda por fazer.
- [x] Gestão robusta de ficheiros temporários em `backend/app/services/file_utils.py` (`try/finally`, feito na Fase 0).
...
- [x] Schemas Pydantic para response (`ProcessResponse`, `ErrorResponse`, `HealthResponse` em `app/models/schemas.py`).
...
- [x] Health check `/healthz` — liveness simples (status + version). Deep check (conectividade OpenRouter) fica para depois.
